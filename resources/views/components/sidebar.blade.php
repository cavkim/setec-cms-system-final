...
<div class="sb">
    ...
    <nav style="flex:1;padding:8px 0;overflow-y:auto">
        ...
        @can('view projects')
            <a href="{{ route('projects.index') }}" class="ni {{ request()->routeIs('projects.*') ? 'on' : '' }}">
                <svg viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2H4z" />
                </svg>
                Projects
                <span
                    class="nb nb-g">{{ \Illuminate\Support\Facades\DB::table('projects')->where('status', 'in_progress')->count() }}</span>
            </a>
        @endcan
        ...
        @can('view tasks')
            <a href="{{ route('tasks.index') }}" class="ni {{ request()->routeIs('tasks.*') ? 'on' : '' }}">
                ...
                Tasks
                <span
                    class="nb nb-r">{{ \Illuminate\Support\Facades\DB::table('tasks')->whereIn('status', ['pending', 'in_progress'])->count() }}</span>
            </a>
        @endcan
    ...
    <div class="sb-foot">
        <div class="u-card">
            <div class="av"
                style="width:30px;height:30px;font-size:11px;background:linear-gradient(135deg,#1565C0,#42A5F5)">
