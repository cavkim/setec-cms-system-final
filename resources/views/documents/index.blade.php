@extends('layouts.app')
@section('title', 'Documents — BuildScape CMS')
@section('page-title', 'Documents')

@section('styles')
<style>
    #doc-drawer { transform: translateX(100%); transition: transform 0.3s cubic-bezier(0.4,0,0.2,1); }
    #doc-drawer.open { transform: translateX(0); }
    #doc-drawer-overlay { opacity:0; pointer-events:none; transition: opacity 0.3s ease; }
    #doc-drawer-overlay.open { opacity:1; pointer-events:auto; }
    .doc-row-active { background-color: rgba(173,198,255,0.08) !important; box-shadow: inset 2px 0 0 #adc6ff; }
</style>
@endsection
...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
    <div>
        <h1 class="text-4xl font-extrabold font-headline tracking-tight text-white mb-1">Documents</h1>
        <p class="text-on-surface-variant">Central repository for all project blueprints, safety permits, and legal contracts.</p>
    </div>
</div>
...
<div class="bg-surface-container-low rounded-xl p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="relative">
        <form method="GET" action="{{ route('documents.index') }}">
            ...
            <input class="bg-surface-container-lowest border border-white/5 rounded-xl pl-10 pr-4 py-2 text-sm text-on-surface w-64 focus:outline-none focus:border-primary placeholder:text-on-surface-variant"
...
<div class="bg-surface-container rounded-2xl overflow-hidden shadow-2xl">
    {{-- Column headers --}}
    <div class="grid grid-cols-12 px-6 py-3 text-[0.6875rem] font-black uppercase tracking-widest text-on-surface-variant bg-surface-container-high/30 border-b border-white/5">
    ...
    <div class="divide-y divide-white/5">
        ...
        <div class="grid grid-cols-12 items-center px-6 py-5 hover:bg-surface-container-high transition-all duration-200 group relative cursor-pointer doc-row"
            ...
            <div class="col-span-5 flex items-center gap-4">
                <div class="w-12 h-12 bg-surface-container-lowest rounded-lg flex items-center justify-center border border-outline-variant/10 flex-shrink-0">
                    <span class="material-symbols-outlined {{ $tm['color'] }} text-2xl" style="font-variation-settings:'FILL' 1;">{{ $tm['icon'] }}</span>
                </div>
                <div>
            ...
            <div class="col-span-2">
                <span class="px-3 py-1 rounded-full text-[0.6rem] font-bold uppercase tracking-wider {{ $tm['bg'] }}">{{ ucfirst($doc->document_type) }}</span>
            </div>
            <div class="col-span-2">
                <p class="text-xs font-semibold text-on-surface">{{ \Carbon\Carbon::parse($doc->created_at)->format('M d, Y') }}</p>
            ...
            <div class="col-span-1 text-center">
                <span class="bg-surface-container-lowest border border-outline-variant/30 px-2 py-0.5 rounded text-[0.65rem] font-bold text-on-surface-variant">v{{ $doc->version_number ?? 1 }}</span>
            </div>
            <div class="col-span-1 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity" onclick="event.stopPropagation()">
                <a href="{{ route('documents.download', $doc->id) }}" class="p-2 hover:bg-primary/20 text-primary transition-colors rounded-lg">
                    <span class="material-symbols-outlined text-xl">download</span>
                </a>
                @can('delete documents')
                <form method="POST" action="{{ route('documents.destroy', $doc->id) }}" onsubmit="return confirm('Delete this document?')">
                    @csrf @method('DELETE')
        ...
        <div class="px-6 py-16 text-center">
            <span class="material-symbols-outlined text-5xl text-on-surface-variant/30 block mb-4" style="font-variation-settings:'FILL' 1;">folder_open</span>
            <p class="text-on-surface-variant text-sm mb-4">No documents uploaded yet.</p>
            @can('upload documents')
            <button onclick="openUploadDrawer()" class="px-6 py-3 bg-primary text-on-primary text-sm font-bold rounded-xl active:scale-95 transition-transform">+ Upload first document</button>
            @endcan
        </div>
        @endforelse
    </div>
    ...
    <div class="px-6 py-4 bg-surface-container-low/50 border-t border-white/5 flex justify-between items-center">
        <span class="text-xs text-on-surface-variant">Showing {{ $documents->firstItem() }}–{{ $documents->lastItem() }} of {{ $documents->total() }}</span>
        <div class="flex gap-2">
            @if(!$documents->onFirstPage())
            <a class="px-3 py-1 text-xs font-bold bg-surface-container-highest text-on-surface-variant rounded-md border border-outline-variant/20" href="{{ $documents->previousPageUrl() }}">Previous</a>
            @endif
            @if($documents->hasMorePages())
            <a class="px-3 py-1 text-xs font-bold bg-primary text-on-primary rounded-md" href="{{ $documents->nextPageUrl() }}">Next</a>
            @endif
        </div>
    </div>
    @endif
...
<div class="fixed bottom-8 right-8 z-50 flex flex-col items-end gap-3 group">
    <span class="pointer-events-none opacity-0 group-hover:opacity-100 transition-all duration-200 translate-x-2 group-hover:translate-x-0 bg-surface-container-highest text-white text-xs font-semibold px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap border border-white/10">Upload Document</span>
    <button onclick="openUploadDrawer()" class="w-14 h-14 rounded-full bg-primary text-on-primary shadow-[0_4px_24px_rgba(77,142,255,0.45)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-200" aria-label="Upload Document">
...
{{-- Document Drawer --}}
<div id="doc-drawer" class="fixed top-0 right-0 h-full w-full max-w-lg bg-surface-container-low shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-[70] flex flex-col border-l border-white/5">
    {{-- Header --}}
    <div class="px-6 py-6 border-b border-white/5 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-on-surface font-headline leading-tight" id="dd-title">Document Details</h2>
                <p class="text-[0.65rem] text-on-surface-variant uppercase tracking-widest font-bold" id="dd-subtitle">Details</p>
            </div>
        </div>
        <button class="w-10 h-10 flex items-center justify-center rounded-full hover:bg-surface-variant transition-colors text-on-surface-variant" onclick="closeDocDrawer()">
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-8" id="dd-view-body">
        <section>
            <h3 class="text-[0.65rem] uppercase tracking-[0.2em] font-black text-on-surface-variant mb-4">File Metadata</h3>
    ...
    <div class="flex-1 overflow-y-auto p-6 hidden" id="dd-upload-body">
        <form id="dd-upload-form" method="POST" action="{{ route('documents.store') }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">Document Name <span class="text-red-400">*</span></label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">Project <span class="text-red-400">*</span></label>
                        <select name="project_id" required class="w-full bg-surface-container border border-white/10 rounded-xl px-3 py-3 text-sm text-white focus:outline-none focus:border-primary">
                            <option value="">— Select project —</option>
                            @foreach($projects as $p)<option value="{{ $p->id }}">{{ $p->project_name }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">Type <span class="text-red-400">*</span></label>
                        <select name="document_type" required class="w-full bg-surface-container border border-white/10 rounded-xl px-3 py-3 text-sm text-white focus:outline-none focus:border-primary">
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">File <span class="text-red-400">*</span></label>
                    <label class="flex flex-col items-center justify-center gap-2 p-8 border-2 border-dashed border-white/10 rounded-xl hover:border-primary/40 hover:bg-primary/5 transition-all cursor-pointer">
                        <span class="material-symbols-outlined text-3xl text-on-surface-variant">upload_file</span>
                        <span class="text-xs font-bold text-on-surface-variant">Click to choose file or drag & drop</span>
                        <span class="text-[10px] text-on-surface-variant/60">PDF, DOC, DWG, images — max 50MB</span>
                        <input type="file" name="file" required accept=".pdf,.doc,.docx,.dwg,.jpg,.jpeg,.png,.xlsx,.zip" class="hidden">
                    </label>
                </div>
                <div>
    ...
    <div class="p-6 bg-surface-container border-t border-white/5 flex gap-3" id="dd-footer-view">
        <a id="dd-download-btn" href="#" class="flex-1 bg-primary text-on-primary py-3 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:brightness-110 transition-all">
            <span class="material-symbols-outlined text-sm">download</span> Download
        </a>
        <button onclick="closeDocDrawer()" class="bg-surface-container-highest text-on-surface border border-outline-variant/30 w-12 h-12 rounded-xl flex items-center justify-center hover:bg-surface-bright transition-colors">
            <span class="material-symbols-outlined">close</span>
        </button>
    </div>
    ...
    <div class="p-6 bg-surface-container border-t border-white/5 flex gap-3 hidden" id="dd-footer-upload">
        <button type="submit" form="dd-upload-form" class="flex-1 bg-primary text-on-primary py-3 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2">
            <span class="material-symbols-outlined text-sm">upload</span> Upload Document
        </button>
        <button type="button" onclick="closeDocDrawer()" class="bg-surface-container-highest text-on-surface border border-outline-variant/30 w-12 h-12 rounded-xl flex items-center justify-center hover:bg-surface-bright transition-colors">
            <span class="material-symbols-outlined">close</span>
        </button>
    </div>
    @endcan
